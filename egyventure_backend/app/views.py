...
def post_interests(request):
    if request.method =='POST':
        try:
            ...
            return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
    return JsonResponse({'error': 'wrong method type'}, status=500)
...
def rate(request):
    try:
        ...
        try:
            ...
            return JsonResponse({'error': f'Error validating user or attraction. {str(e)}'}, status=500)
...
def bert(request):
    ...
    try:
        recommendations = sbert_reviews.main(attractions_db, reviews_db, target_user_id)
    except Exception as e:
