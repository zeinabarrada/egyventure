...
def get_all_reviews_df(reviews_collection):
...
def main(target_user_id):
    ...
    reviews_df = get_all_reviews_df(reviews_collection)
        ...
        if similarity_scores:
            ...
            if recommendations:
                for rec in recommendations:
                    print(
                        f"Attraction: {rec['attraction_name']} (ID: {rec['attraction_id']}) - Score: {rec['score']:.4f}")
            else:
                print("Could not find any new attractions to recommend.")
        else:
            print(
                f"Could not calculate similarity for user {target_user_id}. They may have no reviews or no common attractions with other users.")
    else:
